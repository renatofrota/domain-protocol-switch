...
echo -e "\n\tdomain-protocol-switch - v0.0.3 - https://github.com/renatofrota/domain-protocol-switch\n";
...
echo "Running!"
find $folder -type f ! -regex '.*\.pdf' ! -regex '.*\.log' ! -regex '.*\.frs' ! -regex '.*\.wpress' ! -regex '.*\.ta?r?\.?g?z?' ! -regex '.*\.sql\.?g?z?' -exec grep -l -E "https?://(www\.)?${domain1}" '{}' \; | xargs -I % sh -c "echo 'Found domain name on %, processing...' ; sed -i -e 's|https\?://\(www\.\)\?${domain1}|http${protocol}://${prefix}${domain2}|g' % " ;
echo "All done!"
killme() {
